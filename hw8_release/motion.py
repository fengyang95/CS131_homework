...
def iterative_lucas_kanade(img1, img2, keypoints,
    ...
    for y, x, gy, gx in np.hstack((keypoints, g)):
        ...
        ### YOUR CODE HERE
        Ixx = np.sum(np.multiply(Ix[y1 - w:y1 + w + 1, x1 - w:x1 + w + 1], Ix[y1 - w:y1 + w + 1, x1 - w:x1 + w + 1]))
        Ixy = np.sum(np.multiply(Ix[y1 - w:y1 + w + 1, x1 - w:x1 + w + 1], Iy[y1 - w:y1 + w + 1, x1 - w:x1 + w + 1]))
        Iyy = np.sum(np.multiply(Iy[y1 - w:y1 + w + 1, x1 - w:x1 + w + 1], Iy[y1 - w:y1 + w + 1, x1 - w:x1 + w + 1]))
        G = np.array([[Ixx, Ixy], [Ixy, Iyy]])
        ### END YOUR CODE
        ...
        for k in range(num_iters):
            ...
            if 0 <= y2 < height and 0 <= x2 < width:
                deltaIk = img1[y1, x1] - img2[y2, x2]
            else:
                continue
            deltax = deltaIk * np.sum(Ix[y1 - w:y1 + w + 1, x1 - w:x1 + w + 1])
            deltay = deltaIk * np.sum(Iy[y1 - w:y1 + w + 1, x1 - w:x1 + w + 1])
            bk = np.array([deltax, deltay])
...
def pyramid_lucas_kanade(img1, img2, keypoints,
    ...
    for L in range(level, -1, -1):
        ### YOUR CODE HERE
        now_keypoints = keypoints / pow(2, L)
        d = iterative_lucas_kanade(pyramid1[L], pyramid2[L], keypoints=now_keypoints, g=g)
        g = L * (g + d)
        ### END YOUR CODE
