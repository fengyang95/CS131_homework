...
def partial_x(img):
    ...
    out = None
    ### YOUR CODE HERE
    Dx = np.array([[0,0,0],[-0.5,0,0.5],[0,0,0]], dtype=np.float32)
    out = conv(img, Dx)
    ### END YOUR CODE
...
def partial_y(img):
    ...
    ### YOUR CODE HERE
    Dy = np.array([[0,-0.5,0],[0,0,0],[0,0.5,0]], dtype=np.float32)
    out = conv(img, Dy)
    ### END YOUR CODE
...
def non_maximum_suppression(G, theta):
    ...
    pad_width = ((1,1), (1,1))
    padded = np.pad(G, pad_width, mode='constant')
    ### BEGIN YOUR CODE
    for i in range(H):
        for j in range(W):
...
def hough_transform(img):
    ...
    for i in range(len(ys)):
        for theta in thetas:
            rho = xs[i]*np.cos(theta)+ys[i]*np.sin(theta)
            rho_index =(int)(np.floor(rho+diag_len))
            theta_index = (int)(np.floor(np.rad2deg(theta))+90)
